    ...
    <div class="goods">
      <div class="detail">
        ...
        <div class="price">
          ...
          <div class="num">
            <p style="margin-top:0.59rem">￥{{salePrice}}</p>
            <p>x1</p>
          </div>
        </div>
      </div>
  ...
  methods: {
    getDetail() {
      ...
      detail({
        ...
        this.goodsInfo.categoryList.forEach((item) => {
          if (item.id === this.cateId) {
            this.cateName = item.name;
            this.salePrice = item.price;
          }
        })
      })
    },
    ...
    switchChange() {
      let vm = this
      // if (vm.isDeduct) {
      //   vm.salePrice = Number(vm.goodsInfo.salePrice - vm.discount).toFixed(2)
      // } else {
      //   vm.salePrice = vm.goodsInfo.salePrice
      // }
      if (vm.isDeduct) {
        vm.salePrice = Number(vm.salePrice - vm.discount).toFixed(2)
      } else {
        vm.salePrice = vm.salePrice
      }
    }
  },
